function [x_d, x_r, y_r, z_r] = getDesiredReference(mpc, boolPlot, nTraj, mD)
    ...
    switch nTraj
        case 1
            ...
            y_r = ppval(cs, x_r);
            z_r = [linspace(0,8,mpc.duration)];
            y_prime = ppval(fnder(cs,1),x_r);
            prime = [ones(size(y_prime)); y_prime; ones(size(y_prime))];
            ...
            for t = 1:mpc.duration
                ...
                x_d(:,t) = reshape([[x_r(t);y_r(t);z_r] + R * x_dr(:,1:mpc.N); repmat(prime(:,t),1,mpc.N)],mpc.nx,1);
            end
        otherwise
            x_dr = [-1/sqrt(2), 1/sqrt(2), 1/sqrt(2), -1/sqrt(2);
                1/sqrt(2), 1/sqrt(2), -1/sqrt(2), -1/sqrt(2);
                0, 0, 0 ,0];
            x_dr = repmat(x_dr,1,10);
            ...
            z_r = [0 * linspace(0,8,mpc.duration)];
            ...
            for t = 1:mpc.duration
                ...
                R = roty(rad2deg(-gamma)) * rotz(rad2deg(-psi));
                x_d(:,t) = reshape([[x_r(t);y_r(t);z_r(t)] + R * x_dr(:,1:mpc.N); repmat(prime(:,t),1,mpc.N)],mpc.nx,1);
            end
    end
    ...
    if (boolPlot)
        if ~mD
            ...
            figure();
            for t = 1:mpc.duration
                plot3(x_r(t), y_r(t),z_r(t),'-or','MarkerSize',5,'MarkerFaceColor','r')
                hold on;
                plot3(x_d(1,t),x_d(2,t),x_d(3,t),'-or','MarkerSize',5,'MarkerFaceColor','k');
                if mpc.N > 1
                    plot3(x_d(7,t),x_d(8,t),x_d(9,t),'or','MarkerSize',5,'MarkerFaceColor','b');
                end
                if mpc.N > 2
                    plot3(x_d(13,t),x_d(14,t),x_d(15,t),'or','MarkerSize',5,'MarkerFaceColor','g');
                end
                if mpc.N > 3
                    plot3(x_d(19,t),x_d(20,t),x_d(21,t),'or','MarkerSize',5,'MarkerFaceColor','y');
                end
                pause(0.4)
            end
        end
